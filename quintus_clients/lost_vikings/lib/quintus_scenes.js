...
Quintus.Scenes = function(Q) {
  ...
  Q.Scene = Q.Class.extend({
    init: function(sceneFunc,opts) {
      this.opts = opts || {};
      this.sceneFunc = sceneFunc; // gets called with a Stage as first argument
    }
  });
  ...
  Q.Stage = Q.GameObject.extend({
    ...
    search: function(obj,collisionMask) {
      ...
      if(this._collisionLayer && (this._collisionLayer.p.type & collisionMask)) {
        this._collisionLayer.resetTileFlags(); // clear out tile flags
    ...
    _locateObj: { // simple non-Sprite locateObj
        p: { 
          x: 0,
          y: 0,
            cx: 0,
    ...
    locate: function(x,y,collisionMask,w,h) {
      var col = null;

      if (Q.debug & Q._DEBUG_RENDER_OFTEN_AND_LOCATE && !this._locateObj.p.name) {
        this._locateObj = new Q.Sprite({
          name: "_locateObj",
          asset: "empty_sprite.png",
          frame: 0,
          type: Q._SPRITE_NONE,
         });
         this.insert(this._locateObj);
      }

      this._locateObj.p.x = x;
      this._locateObj.p.y = y;
      this._locateObj.p.w = (w || 1);
      this._locateObj.p.h = (h || 1);
      collisionMask = (collisionMask || Q._SPRITE_DEFAULT);
      this._locateObj.p.cx = this._locateObj.p.w / 2; // adjust correct p.cx and p.cy
      this._locateObj.p.cy = this._locateObj.p.h / 2;

      if (Q.debug & Q._DEBUG_RENDER_OFTEN_AND_LOCATE) {
        this._locateObj.size(true, true); // adjust correct p.cx and p.cy
        Q._generatePoints(this._locateObj, true); // force the points to be recalculated automatically from size of Sprite (since size changed)
        this.regrid(this._locateObj,true);
        renderAllForDebug(Q, this._locateObj);
      }
      else {
        Q._generatePoints(this._locateObj, true); // force the points to be recalculated automatically from size of Sprite (since size changed)
        this.regrid(this._locateObj,true);
    ...
    collide: function(obj, options) {
      var col, col2, collisionMask, 
          maxCol, curCol, skipEvents, skipReciprocalEvents;
      if(Q._isObject(options)) {
        collisionMask = options.collisionMask;
        maxCol = options.maxCol;
        skipEvents = options.skipEvents;
        skipReciprocalEvents = options.skipReciprocalEvents;
      } else {
        collisionMask = options;
      }
      collisionMask = collisionMask  || (obj.p && obj.p.collisionMask);
      maxCol = maxCol || 2; //2=SVEN; 3=original code;

      Q._generateCollisionPoints(obj);
      this.regrid(obj);

      curCol = maxCol;
      // test against the collision layer first
      if(this._collisionLayer && (this._collisionLayer.p.type & collisionMask)) {
        //var savex,savey, savevx, savevy;
        this._collisionLayer.resetTileFlags(); // clear out tile flags
        while(curCol > 0 && (col = this._collisionLayer.collide(obj))) {

          col.obj = this._collisionLayer;

          //if (Q.debug & Q._DEBUG_LOG_COLLISIONS && ((!Q._DEBUG_LOG_COLLISIONS_AS) || Q._DEBUG_LOG_COLLISIONS_AS == obj.p.name)) {
          //    savex = obj.p.x;
          //    savey = obj.p.y;
          //    savevx = obj.p.vx;
          //    savevy = obj.p.vy;
          ...
          //if (Q.debug & Q._DEBUG_LOG_COLLISIONS && ((!Q._DEBUG_LOG_COLLISIONS_AS) || Q._DEBUG_LOG_COLLISIONS_AS == obj.p.name))
          //    console.log("collision-layer col: tile="+col.tile+"("+col.tileX+"/"+col.tileY+") x="+savex+"->"+obj.p.x+" y="+savey+"->"+obj.p.y+" vx="+savevx+"->"+obj.p.vx+" vy="+savevy+"->"+obj.p.vy+" Nx="+col.normalX+" Ny="+col.normalY+" Sx="+col.separate[0]+" Sy="+col.separate[1]);

          curCol--;
        }
      }

      // then against other sprites
      curCol = maxCol;
      while(curCol > 0 && (col2 = this.gridTest(obj, collisionMask))) {//,this._collisionLayer))) {
        ...
        // Do the reciprocal collision
        // TODO: extract
        if(! (skipEvents || skipReciprocalEvents)) { 
          var obj2 = col2.obj;
          col2.obj = obj;
          col2.normalX *= -1;
          col2.normalY *= -1;
    ...
    // gets called each animation-frame by the GameLoop
    ...
    // gets called each animation-frame by the GameLoop (after 'step' is called on all Stages)
    // - calls render on all its Sprites
    render: function(ctx) {
