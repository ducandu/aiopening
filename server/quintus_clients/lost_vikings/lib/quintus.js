...
var Quintus = function Quintus(opts) {
  ...
  Q._DEBUG_OFF                        = 0x0000;
  Q._DEBUG_LOG_COLLISIONS             = 0x0001; // log collision algorithm results and decisions to console
      Q._DEBUG_LOG_COLLISIONS_AS         = "Baleog"; // only log everything for p.name==x
  Q._DEBUG_CLEAR_CONSOLE              = 0x0002;
  Q._DEBUG_LOG_ANIMATIONS             = 0x0004; // log animations to console
  Q._DEBUG_RENDER_SPRITES_PINK        = 0x0008; // paint pink border around where collision points are for each sprite
  Q._DEBUG_RENDER_SPRITES_FILL        = 0x0010; // fill all sprites
  Q._DEBUG_RENDER_COLLISION_LAYER     = 0x0020; // paint the collision layer (but not the other layers!)
  Q._DEBUG_RENDER_OFTEN               = 0x0040; // render everything after every sprites' step AND collision methods
    Q._DEBUG_RENDER_OFTEN_AND_LOCATE  = 0x0080; // only works with RENDER_OFTEN: also render the locate objects
  ...
  // normalizes the x/y and w/h properties of the incoming object from tile units to pixels
  Q._xywhTileToPix = function(p, tileW, tileH) {
    if (! tileW) tileW = _TILE_SIZE;
    if (! tileH) tileH = tileW;
    p.x = (p.x+p.w/2)*tileW;
    p.y = (p.y+p.h/2)*tileH;
    Q._whTileToPix(p, tileW, tileH);
  };

  // normalizes the x/y and w/h properties of the incoming object from tile units to pixels
  Q._whTileToPix = function(p, tileW, tileH) {
    if (! tileW) tileW = _TILE_SIZE;
    if (! tileH) tileH = tileW;
    p.w = p.w*tileW;
    p.h = p.h*tileH;
  ...
  Q.gameLoop = function(callback) {
    ...
    Q.gameLoopCallbackWrapper = function() {
      if ((Q.debug & Q._DEBUG_LOG_COLLISIONS && Q.debug & Q._DEBUG_CLEAR_CONSOLE) && !(Q._loopFrame % 50))
      {
        console.clear();
      }
      ...
      var dt;
      if (Q.debug & Q._DEBUG_RENDER_OFTEN) dt = (1000/60); // fake dt to account for debugging activity
      else dt = now - Q.lastGameLoopFrame;
      ...
      if (Q.debug & Q._DEBUG_RENDER_OFTEN) Q.lastGameLoopFrame += dt;
      else Q.lastGameLoopFrame = now;
    };
  ...
  Q.Evented.extend("GameObject",{
    ...
    add: function(component, params) {
      //component = Q._normalizeArg(components);
      if(!this.activeComponents) { this.activeComponents = []; }
      //for(var i=0,len=components.length;i<len;i++) {
        //var name = components[i],
        var Comp = Q.components[component];
        if(!this.has(component) && Comp) { 
          var c = new Comp(this, params); 
  ...
  Q.GameObject.extend("GameState",{
    ...
    _triggerProperty: function(value,key) {
      ...
      if(old !== value) {
        this.p[key] = value;
        this.trigger("change."+key,[value, old]);
      }
    },
    ...
    // Decrement an individual property by amount
  ...
  Q.launchFullscreen = function(element) {
  ...
  Q.setup = function(id, options) {
    ...
    if (! options.recall) {
      Q.debug = options.debugLevel || 0;
      requestAnimationFrameShim(); // only after knowing the debug level (requestAnimationFrame needs to be switched off when debug-rendering very often)
    }

    id = id || (Q.el ? Q.el.id : 0) || "quintus";
    ...
    if(!Q.el) {
      ...
      Q.el.id = id;
      Q.el.width = Math.min((options.width || document.documentElement.clientWidth), maxWidth);
      Q.el.height = Math.min((options.height || document.documentElement.clientHeight), maxHeight);
    ...
    Q.launchFullscreen(document.documentElement); // the whole page
    //launchFullScreen(document.getElementById("videoElement")); // any individual element


    if (! options.recall) {
      var react = function(e) {
        Q.reactToResize(Math.min(document.documentElement.clientWidth, maxWidth), maxWidth, Math.min(document.documentElement.clientHeight, maxHeight), maxHeight);
      };
      window.addEventListener('resize', react);
      window.addEventListener('orientationchange', react);
    }
  ...
  Q.reactToResize = function(w, maxW, h, maxH) {
    clearTimeout(Q.resizeTimer);
    Q.resizeTimer = setTimeout(function() {Q.setup({recall: 1, width: w, height: h, maxWidth: maxW, maxHeight: maxH})}, 1000);
  };
  ...
  Q.load = function(assets,callback,options) {
    ...
    var loadedCallback = function(key,obj,force) {
      ...
      if(assetsRemaining === 0 && callback) {
        ...
        //console.log("   assets "+assets+" all loaded -> calling callback");
        callback.apply((options.context || Q), options.callbackArgs); 
      }
    };
    ...
    Q._each(assetObj,function(itm,key) {
      ...
      //console.log("   loading key="+key+" (type="+assetType+")");
...
function requestAnimationFrameShim() {
    ...
    if ((!window.requestAnimationFrame) || Q.debug & Q._DEBUG_RENDER_OFTEN) {
    ...
    if ((!window.cancelAnimationFrame) || Q.debug & Q._DEBUG_RENDER_OFTEN) {
