...
Quintus.Anim = function(Q) {
  ...
  // define animation flags that control, which animations allow/disallow which things
  // - that way, we can determine some character state from the character's current animation
  Q.ANIM_NONE                  =    0x0;
  Q.ANIM_SWING_SWORD           =    0x1;
  Q.ANIM_DISABLES_CONTROL      =    0x2;
  Q.ANIM_PROHIBITS_STAND       =    0x4;  // anims that should never be overwritten by 'stand' (even though player might not x-move)
  Q.ANIM_BOW                   =    0x8;
  ...
  Q.component('animation',{
    ...
    extend: {
      ...
      blink: function(rate, duration) {
        this.animation.blink(rate, duration);
      }
    },
    step: function(dt) {
      ...
      if (p.blinkDuration > 0) {
        p.blinkTime += dt;
        // blinking stops
        if (p.blinkTime >= p.blinkDuration) {
          p.blinkDuration = 0;
          p.hidden = false;
        }
        else {
          var frame = Math.floor(p.blinkTime * p.blinkRate);
          p.hidden = (frame % 2 ? true : false);
        }
      }
      // animation stuff?
      if(p.animation) {
    ...
    play: function(name,priority) {
      ...
      if(name !== p.animation && priority >= p.animationPriority) {
        ...
        p.animationFlags = anim.flags || Q.ANIM_NONE;
        p.animationKeysStatus = anim.keysStatus || Q._EMPTY_OBJ;
    ...
    blink: function(rate /*in 1/s*/, duration /*in s*/) {
      var entity = this.entity, p = entity.p;
      p.blinkRate = (rate || 3);
      p.blinkDuration = (duration || 3);
      p.blinkTime = 0;
    },
