...
Quintus.Input = function(Q) {
  ...
  Q.component("shineGridWorldControls", {
    ...
    added: function() {
      ...
      //aiopener.lastAiAction = -1; // -1 == none; 0=no action; 1=up, 2=right, 3=down, 4=left
      ...
      //aiopener.isTerminated = false; // did we just terminate an episode?
      shine.a = 0;
    ...
    process_sars: function(a, r, s_){
	    ...
	    if (shine.sars.length == 1) {
            //console.log("finishing old sars: s="+aiopener.sars[0]+" a="+a+" r="+LAST_REWARD+" s'="+s_);
    ...
    step: function(dt) {
      ...
      if (shine.a == 0) {
          ...
          if (p.diffY || p.diffX) {
              ...
              //aiopener.currentStep = aiopener.frameCount;
              p.origX = p.x;
              p.origY = p.y;
              p.destX = p.x + p.diffX;
              p.destY = p.y + p.diffY;
