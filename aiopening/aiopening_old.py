...
import gym  # use openAI gym as base framework for all environments
import random
...
class GameLab(spyg.Game):
    """
    a spyg.Game child that can execute experiments on MDPs (Levels) of the Game
    - experiments can only be done on MDPs (which are children of spyg.Level), not on plain Levels
    - one can add Algorithm and Model objects to the GameLab
    - TODO: should have all kinds of tools to automate finding good Models (topologies, hyperparameters) and Algorithms to solve the different MDPs
    ...
    :param int frames_per_action: the number of frames that each action will 'last' (each action picked by the algos lasts for that many frames (is repeated))
                                  e.g.: frames_per_action==2 -> algo gets to pick a new action (a) every 2 frames; a will be replayed 2 frames in a row
    :param str render_mode: the rendering mode for the experiments (by default, rendering is switched off for performance reasons (experiments usually
                            run automatically on a server anyway))
    :param int debug_flags: a bitmap for setting different debug flags (see global variables DEBUG_...)
    """
    def __init__(self, width: int=0, height: int=0, title: str="aiopening Demo!", max_fps: int=60,
                 frames_per_action=4, render_mode="off", debug_flags=DEBUG_NONE):
        super().__init__(width, height, title, max_fps, debug_flags)
...
class MDP(spyg.Level, metaclass=ABCMeta):
    ...
    - any GameLabs hat would like to do RL via Algorithm objects on an MDP, need to use MDP Levels, not plain spyg.Levels
      -- options to the MDP objects can be given via the spyg.GameLab c'tor
    ...
    def __init__(self, name, **kwargs):
        ...
        self.frames_per_action = kwargs.get("frames_per_action", 4)  # the fps parameter has to be set in the GameLab c'tor


class Algorithm(object, metaclass=ABCMeta):
    """
    an RL algorithm that gets added to a spyg.GameLab and can then solve different MDPs
    - may need one or more Model objects
    - can reset, retrain, init, ... the Model(s)
    - can request inputs from the GameLab (e.g. via the spyg.Display object of the GameLab)
    - selects actions during learning and passes its decisions on to the MDP for execution

    :param str name: the name of the Algorithm object
    """
    def __init__(self, name="Algo1"):
        self.name = name

    @abstractmethod
    def run(self, options: dict={}) -> None:
        """
        'runs' this algorithm with the given options. Running could mean to either start training a model (initialize + start) or to continue training
            an already pre-trained model, etc..
        :param dict options: the options to use for this particular Algorithm. Can include Models to use or hyperparameters/topologies to apply to the modules.
        """
        pass
...
class Model(object, metaclass=ABCMeta):
    """
    a simple Model abstract class. A Model could be a NN, a Q-lookup-table, etc..
    - Model objects can be used by Algorithm objects for running these Algorithms on MDPs

    :param str name: the name of the Model object
    """
    ...
    def reset_parameters(self) -> None:
        """
        resets all "weights" of this Model (but not the Model's current hyperparameters)
        """
        pass
    ...
    def set_hyperparameters(self, **kwargs) -> None:
        """
        sets the Model's hyperparameters by keyword
        :param any kwargs: key/value pairs for new hyperparameter settings. Any hyperparameters not in kwargs will be left as is
        """
        pass


class AlgorithmOld(object, metaclass=ABCMeta):
    """
...
class BasicQLearner(AlgorithmOld, metaclass=ABCMeta):
    """
